...
- Launch a t3a.medium (Nexus needs 8 GB of RAM) EC2 instance using the Amazon Linux 2023 AMI with security group allowing `SSH (22), Nexus Port (8081), 8082 and 8083` connections.
...
```bash
cd /opt
sudo wget -O nexus.tar.gz https://download.sonatype.com/nexus/3/nexus-3.68.1-02-java11-unix.tar.gz
ls
```
...
<settings>
  <mirrors>
    <mirror>
      ...
      <id>nexus</id>
      <mirrorOf>*</mirrorOf>
      <url>http://nexus-server public ip:8081/repository/maven-proxy-hands-on/</url>
    </mirror>
  </mirrors>
  <profiles>
    <profile>
...
<settings>
  <mirrors>
    <mirror>
      ...
      <id>nexus</id>
      <mirrorOf>*</mirrorOf>
      <url>http://nexus-server public ip:8081/repository/maven-public/</url>
    </mirror>
  </mirrors>
  <profiles>
    <profile>
...
<settings>
  <mirrors>
    <mirror>
      ...
      <id>nexus</id>
      <mirrorOf>*</mirrorOf>
      <url>http://nexus-server public ip:8081/repository/maven-all/</url>
    </mirror>
  </mirrors>
  <profiles>
    <profile>
...
## Part 8: Install Docker on Amazon Linux 2023 EC2 Instance
...
```bash
docker login -u admin -p admin 52.23.200.75:8082 # don't forget to change password.
docker pull 52.23.200.75:8082/alpine
docker image ls
```
...
```bash
docker login -u admin -p admin 52.23.200.75:8083
docker pull 52.23.200.75:8082/nginx
docker image ls
docker tag 52.23.200.75:8082/nginx 52.23.200.75:8083/myng
docker push 52.23.200.75:8083/myng
```
