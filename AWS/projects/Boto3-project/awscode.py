...
'''
Define the tag/key values which will determine if the instance needs to be stopped and/or started. If the instance doesn't have these tags, the scheduler will ignore the instance.
'''
...
'''
Function to takes specific key as input and determince the associated value if it exists in the instance tags
'''
...
def process_instance(instance, current_hour):
  ...
  print(f"Checking the instance {instance_id}")
  ...
  if do_nothing:
    result = f"Instace {instance_id} is {instance_state}, Therefore no action required at this time."
...
def process_instances():
  ...
  if instances is None:
    return_status = f"No instances found."
...
'''
Check whether the versioning is enabled on the specified bucket.
...
def process_bucket(bucket_name):
  ...
  if is_enabled == "Enabled":
    result =f"Versioning is enabled on the bucket {bucket_name}"

  else:
    versioning_configuration = {"Status": "Enabled"}
    s3.put_bucket_versioning(Bucket=bucket_name, VersioningConfiguration=versioning_configuration)

    result = f"Enabling versioning on the bucket {bucket_name}"
...
def lambda_handler(event, context):
  ...
  try:
    ...
    return_code = 500
    return_status = f"Unexpected error ocurred: {e}"
  finally:
    print(return_status)
    return {"statusCode": return_code, "body": json.dumps(return_status)}

'''
This is put in for testing from the IDE. If this function is running as a Lambda function, then __name__ will be "__main__" and we can brute force call the lambda handler.
'''
